...
3. Install Ansible and run the full playbook:
   ...
   ```
   This installs Docker, pulls the RaspAP Docker image and starts it, pulls/builds ROS 2 images, runs `npm install` for the command center, and configures everything.
...
If you have **Docker Desktop**: put a Raspberry Pi OS 64-bit Lite `.img` or `.img.xz` in `image/`, then from the repo root run **`./image/build-with-docker.sh`**. The script **automatically expands the image by 4 GiB** and grows the root partition, so you do **not** need Raspberry Pi Imager to increase storage. Output: **`image/maser_buoy_build.img`**. **RaspAP runs as a Docker container** (`ghcr.io/raspap/raspap-docker`). The build script **pre-pulls** the RaspAP image and bakes it into the image, so **first boot needs no network**: the playbook loads the tarball and starts the container. WiFi (hostapd/dnsmasq) and the .buoy DNS config are inside the container; the command center stays on the host. Docker Compose images for ROS are not pre-built in chroot; first boot or a later run with network will pull/build them. If the script fails with "Could not create loop device" (e.g. on Mac), use Linux/WSL2 or see **[BUILD-QEMU.md](BUILD-QEMU.md)** for manual steps.

## Deploy (no internet)

Flash the built image to each Pi 5, insert the SD, and power on **without network**. The first-boot service runs the playbook with `offline_first_boot: true`, which loads the baked RaspAP image (if present), starts the RaspAP container and other services (hostname, iptables, ROS Docker Compose, command center). RaspAP and Docker are already on the image; the RaspAP container provides WiFi and .buoy DNS. The first_boot role then disables the service so it does not run again on the next boot.
...
4. **SSH in:** `ssh <user>@<ip>`
   - **User:** whatever your **base image** uses. If you built from Raspberry Pi OS and set a user in Imager, use that. Otherwise the Lite image may have user `pi` (set its password first if needed, e.g. with Imager’s “Set username and password”).
