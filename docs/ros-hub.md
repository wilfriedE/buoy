# Maser Buoy – User guide: connecting and interacting with ROS devices

This guide is for **users** who connect to a Maser Buoy hub (WiFi and web dashboard) and want to run or interact with ROS 2 nodes and devices on the network.
...
- **Rosbridge URL:** `ws://maser.buoy:9090` (or `ws://10.3.141.1:9090` if the hostname does not resolve).
...
**Assumptions:** You are on the hub’s WiFi; ROS domain is `0`; rosbridge is at `ws://maser.buoy:9090`.
...
const ros = new ROSLIB.Ros({ url: 'ws://maser.buoy:9090' });
ros.on('connection', () => {
